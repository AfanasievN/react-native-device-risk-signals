...
The catalog includes exact field types and per-field JSON Schema fragments generated from
`contract/source/signal-types.source.json`, which is derived from the public TypeScript contract and
held in sync with it by `npm run verify:signal-types`. The complete event envelope is published as
